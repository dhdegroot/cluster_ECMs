...
ecms <- read_csv(file.path('data','conversions_iAB.csv'), col_names=FALSE)
...
ecms_unique <- ecms %>% distinct()

# Drop uninteresting ECMs
interesting_ecms <- ecms_unique %>%
  filter(R_NaKt > 0 & M_gal_e == 0 & M_glc__D_e < 0 & M_lac__D_e == 0 & M_lac__L_e == 0 & M_man_e == 0 & M_fru_e ==0)
