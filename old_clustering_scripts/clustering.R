...
ecms <- read.csv('conversions_iAB.csv', header=FALSE)
...
interesting_ecms <- ecms %>%
  filter(objective > 0 & M_gal_e < 0 & M_glc__D_e == 0 & M_mal__L_e == 0 & M_lac__D_e == 0 & M_lac__L_e == 0 & M_man_e == 0)
...
filled_ecms <- interesting_ecms[row_indices,col_indices] %>%
  apply(1, function(x){x / max(abs(x))}) %>% # Rows (ECMs)
  t() %>%
  apply(2, function(x){x / max(abs(x))}) %>% # Columns (metabolites)
  as.data.frame()
...
# Chop ECMs into N parts, to allow later clustering without running out of memory
n_partitions <- 1
ecms_split <- split(filled_ecms, rep(1:n_partitions, length.out = nrow(filled_ecms), each = ceiling(nrow(filled_ecms)/n_partitions)))

# Calculate K-means clusters for each subpart of ECMs (20 clusters per part)
kms <- vector("list", length(ecms_split))
centers <- c()
for (i in 1:length(ecms_split)) {
  print(paste('Doing round', i, 'of', length(ecms_split)))
  x <- kmeans(ecms_split[[i]], 20, iter.max=20)
  kms[[i]] <- x
  centers <- rbind(centers, kms[[i]]$centers)
}

# Order the cluster centers, so we know in what order to show them
norm_centers <- centers %>%
  apply(1, function(x){x / max(abs(x))}) %>%
  t() %>%
  as.data.frame()
cluster.order <- hclust(dist(norm_centers), method = "average")$order # clustering
