...
factor_diff_obj <- 1
shift_pos = 5
log_shift <- .2 # Has to be smaller than 1
BIOMASS_ONE <- TRUE
CLUSTER_TERNARY <- FALSE

# Read the ECMs as row vectors (R convention), and add column names for each metabolite
ecms <- read.csv('data/conversions_ecolicore_hideoutputs.csv', header=TRUE)
...
filled_ecms[filled_ecms<0] <- -log(-(filled_ecms[filled_ecms<0])/(-log_shift*max_neg))
...
inv_get_labels <- function(orig){
  ...
  for(i in 1:length(orig)){
    if (orig[i]<0){
      result[i] = -log(-(orig[i])/(-log_shift*max_neg))
    }else if(orig[i]>0){
...
clustered_ecms %>%
  ...
  scale_fill_gradient2(midpoint = 0, low = paper_colors[1], mid = "grey90",
                       high = paper_colors[3], space = "Lab", breaks=inv_get_labels(c(-10,0,1)),
                       labels=as.character(c(-10,0,1),format='e'),
                       na.value='white',
