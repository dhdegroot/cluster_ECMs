...
ecms <- read_csv('data/iIT_full_copy.csv', col_names=TRUE)

# Drop uninteresting ECMs
interesting_ecms <- ecms %>%
  filter(objective > 0 & M_ala__L_e == 0 & M_ac_e != 0)
...
clust_weights[c('D-Glucose','Ammonium','CO2','Sulfate','Formate')] <- c(100,50,50,30,30)
